...
try:
    ...
    print("{}: Servidor com Broken Pipe.".format(datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S')))
    exit(1)
...
def processar_custom_v():
    ...
    if "include:+/usr/local/etc/unbound/anablock.conf.rj%0D%0A" not in linhas_custom_v:
        linhas_custom_v.append("include:+/usr/local/etc/unbound/anablock.conf.rj%0D%0A")
...
if os.path.isfile(CONF_UNBOUND_T) and os.path.getsize(CONF_UNBOUND_T) > 0:
    ...
    print("{}: API não retornou dados.".format(datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S')))
    exit(1)
...
if output and "no errors" in output.lower():
    ...
    print("{}: Arquivo de configuração Unbound contém erros.".format(datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S')))
    exit(1)
...
try:
    ...
    print("{}: Erro ao executar comando 'technodns recursive rewrite'.".format(datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S')))

try:
    subprocess.check_call(["/usr/local/bin/technodns/technodns", "recursive", "restart"], stdout=subprocess.PIPE)
    print("{}: Tabela de bloqueio atualizada.".format(datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S')))
except subprocess.CalledProcessError as e:
    print("{}: Erro ao executar comando 'technodns recursive restart'.".format(datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S')))
